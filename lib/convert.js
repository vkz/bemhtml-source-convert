...
convert.stx = {
    ...
    toBH: function toBh(src) {throw("STUB: not implemented"); }
};
...
convert.run = function run(options) {
    ...
    function finish(source) {
        ...
        if (options.parse) { pushResult(convert.stx.parse (source));}
        if (options.classify) { pushResult(convert.stx.classify(source));}
